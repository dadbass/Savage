...
client.on('voiceStateUpdate', (old, now) => {
  const channel = client.channels.get('505307557955633152');
...
const OrochiX = new Discord.Client({disableEveryone: true, maxMessagesCache: 1});
const tpoints = JSON.parse(fs.readFileSync('./Text.json', 'UTF8'));
const vpoints = JSON.parse(fs.readFileSync('./Voice.json', 'UTF8'));
OrochiX.on('ready',async () => {
  console.log(`OrochiX Top `);
  OrochiX.users.forEach(m => {
    if(m.bot) return;
...
OrochiX.on('message',async message => {
  ...
  if(message.content.startsWith(prefix + 'top')) {
    ...
    let OrochiX = new Discord.RichEmbed();
    OrochiX.setAuthor(message.author.username, message.author.avatarURL);
    OrochiX.setTitle('# " Top');
 
    OrochiX.addField(`**TOP 5 TEXT 💬**`, _topText, true);
    OrochiX.addField(`**TOP 5 VOICE 🎙**`, _voiceText, true);
    OrochiX.setFooter(`Probot`, message.guild.iconURL);
    message.channel.send(OrochiX).catch(e => {
      if(e) return message.channel.send(`**. Error; \`${e.message}\`**`);
    });
  }
});

OrochiX.on('voiceStateUpdate', (u, member) => {
...
client.on('message', message => {
  ...
  if (message.mentions.users.size < 1) return message.channel.send(":information_source: `#ban @OrochiX` يجب تحديد شخص");
...
user.send(`You Are Has Been Banned Permanently In ${message.guild.name}`)
  }})
  ...
  client.on('message', async message => {
    ...
    .setImage('https://c.top4top.net/p_1029o1te41.png')
      ...
      if(message.content.startsWith(prefix + "mute")) {
          if(!message.member.hasPermission('MUTE_MEMBERS')) return message.channel.send('**انت لست من الادارة :x:**' );
          if(!mutePerson) return message.channel.sendEmbed(muteembed)
          if(mutePerson === message.author) return message.channel.send('**ماتقدر تعطي نفسك ميوت :x:**');
          if(mutePerson === client.user) return message.channel.send('**- ماتقدر تعطي البوت ميوت :x:**');
          if(message.guild.member(mutePerson).roles.has(muteRole.id)) return message.channel.send('**هذا الشخص ميوتد بالفعل :x:**');
          if(!muteRole) return message.guild.createRole({ name: "Muted", permissions: [] });
          if(!time) return message.channel.sendEmbed(muteembed);
          if(!time.match(/[1-60][s,m,h,d,w]/g)) return message.channel.sendEmbed(muteembed)
          if(!muteReason) return muteReason = "**Null**";
          message.guild.member(mutePerson).addRole(muteRole);
          message.channel.send(`**:white_check_mark: ${mutePerson} has been muted ! :zipper_mouth: **`)
          message.delete()
          let muteelogmbed = new Discord.RichEmbed()
          .setTitle('**New Muted User**')
          .addField('Muted User:', `${mutePerson}`)
          .addField('Muted By:', `${message.author}`)
          .addField('Reason:', `${muteReason}`)
          .setTitle(`${message.createdAt}`)
          mutechannel.sendEmbed(muteelogmbed)
  ...
  var command = message.content.toLowerCase().split(" ")[0];
    ...
    if(command == prefix + 'unban') {
      ...
      client.on('message', async message => {
        ...
        if(!banchannel) return message.guild.createChannel("log", 'text').then(c => {
          c.setPosition(1);
      });
        if(message.content.startsWith(prefix + "ban")) {
           if(!message.guild.member(message.author).hasPermission("BAN_MEMBERS")) return message.channel.send("**انت لست من الادارة :x:**");
           if(!User) return message.channel.sendEmbed(banembed)
           if(User.id === client.user.id) return message.channel.send("**ماتقدر تعطي البوت ميوت**");
           if(User.id === message.guild.owner.id) return message.channel.send("**ماتقدر تبند الاونر شيب**");
           if(!time) return message.channel.send("**- اكتب الوقت**");
           if(!time.match(/[1-60][s,m,h,d,w]/g)) return message.channel.send('**هذا الوقت غير صحيح**');
      ...
      let command = message.content.split(" ")[0];
        ...
        let args = message.content.split(" ").slice(1);	 
      if(command === `unmute`) {2
        if(!message.member.hasPermission("MUTE_MEMBERS")) return message.channel.sendMessage("**انت لست من الادارة**").then(m => m.delete(5000));
      if(!message.guild.member(client.user).hasPermission("MUTE_MEMBERS")) return message.reply("**ليس لدى البوت الصلاحيات الكافيه**").then(msg => msg.delete(6000))
...
client.on('message', message => {
  var prefix = "#";
    ...
    if (message.mentions.users.size < 1) return message.channel.send(":information_source: `#kick @OrochiX` يجب تحديد شخص ");
...
#setReply اضافه رساله ورد تلقائي
